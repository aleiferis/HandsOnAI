...
def save_to_db():
    if current_dataset is None:
        return {"message": "No dataset available to save."}
    current_dataset.to_sql("mobility_forecast", con=engine, if_exists="replace", index=False)
    return {"message": "Dataset saved to PostgreSQL database."}
